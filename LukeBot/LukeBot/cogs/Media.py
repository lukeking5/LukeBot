...
class Media(commands.Cog):
    ...
    async def gif(self, ctx, *, arg: str):
        ...
        # grabbing gifs
        response = requests.get(f"https://tenor.googleapis.com/v2/search?q={arg}&key={apiKey}&limit={500}") #load first 500 gifs
    ...
    @commands.hybrid_command(name="wr", description="Get the winrate and other attributes about any LoL champion.")
    async def wr(self, ctx, *, arg):
        """Get the winrate and other attributes about any LoL"""
        newArg = arg.replace(' ', '') #spaces on op.gg website are removed
